...
class ExpenseChatAgent:
    ...
    def _is_rate_limit_error(self, exc: Exception) -> bool:
        """
        Identify Groq throttling responses and similar temporary API failures.
        """
    ...
    def _build_unavailable_message(self) -> ChatTurnResult:
        """
        Return a user-friendly fallback response when the API is temporarily unavailable.
        """
        message = "The expense service is temporarily unavailable. Please try again shortly."
        return ChatTurnResult(
            assistant_text=message,
            usage={},
            raw_assistant_message={"role": "assistant", "content": message},
            rate_limit_failure=True,
        )

    def _parse_retry_after_seconds(self, exc: Exception) -> Optional[float]:
        """
        Extract a suggested wait time from a rate-limit exception message.
        """
        message = str(exc).lower()
        ...
        match = re.search(r"retry after\s*[:]?\s*([0-9]+)", message)
        if match:
            return float(match.group(1))

        return None

    def _sleep_before_retry(self, exc: Exception) -> None:
        wait_seconds = self._parse_retry_after_seconds(exc)
        if wait_seconds is None or wait_seconds <= 0:
            wait_seconds = RETRY_BACKOFF_SECS

        logger.info("Waiting %.1fs before retrying after rate limit", wait_seconds)
    ...
    def _call_model(self, messages: List[Dict[str, Any]]) -> ChatTurnResult:
        ...
        """
        # Attempt streaming with retries for transient throttling errors.
            ...
            try:
                ...
                if self._is_rate_limit_error(exc):
                    if attempt < MAX_RETRIES:
                        logger.warning(
                            "Groq rate limit hit; retrying stream (%d/%d): %s",
                            attempt + 1,
                            MAX_RETRIES,
                            exc,
                        )
                    ...
                    logger.warning(
                        "Groq rate limit hit after %d retries; falling back to non-streaming: %s",
                        attempt,
                        exc,
                    )
                    break
                ...
                return self._retry_non_stream(messages)

        return self._retry_non_stream(messages)

    def _retry_non_stream(self, messages: List[Dict[str, Any]]) -> ChatTurnResult:
        """
        Retry non-streaming completion for transient throttling errors.
        """
        for attempt in range(MAX_RETRIES + 1):
            try:
                ...
                if self._is_rate_limit_error(fallback_exc):
                    if attempt < MAX_RETRIES:
                        logger.warning(
                            "Groq rate limit hit during non-streaming fallback; retrying (%d/%d): %s",
                            attempt + 1,
                            MAX_RETRIES,
                            fallback_exc,
                        )
                        self._sleep_before_retry(fallback_exc)
                        continue

                    logger.warning(
                        "Groq rate limit hit during non-streaming fallback after %d retries; returning fallback: %s",
                        attempt,
                        fallback_exc,
                    )
                    return self._build_unavailable_message()

                logger.exception("Non-streaming completion also failed")
                return self._build_unavailable_message()
    ...
    def chat_once(self, user_input: str) -> ChatTurnResult:
        ...
        for round_index in range(self.max_tool_rounds):
            ...
            # If we already executed tool calls, preserve the success and surface partial failure
            if result.rate_limit_failure and total_tool_calls > 0:
                partial_result = self._build_rate_limit_after_tools_message(total_tool_calls)
                self.messages.append(partial_result.raw_assistant_message)
                last_result.assistant_text = partial_result.assistant_text
                break
